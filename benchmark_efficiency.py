...
def run(args) -> None:
    ...
    notes = [
        "# 效率基准测试",
        "",
        "本目录由 `benchmark_efficiency.py` 自动生成。",
        "",
        "文件说明：",
        "- `benchmark_results.csv`：按实验和 batch size 汇总的延迟与 FLOPs。",
        "- `benchmark_repeats.csv`：每次重复测试的原始记录。",
        "- `summary.csv`：便于排序的完整模型效率表。",
        "- `core_model_efficiency.csv`：四个主任务的 seed42 核心/适配模型效率表。",
        "- `benchmark_results.json`：完整元数据和 FLOPs 分解。",
        "",
        "FLOPs 约定：一次乘加计为 2 个 FLOPs，MACs 按 FLOPs / 2 计算。",
        "汇总表中的延迟、吞吐和显存为多次重复测试的平均值。",
        "显存统计在每个模型和 batch size 的预热计时前重置 CUDA 峰值统计。",
        "Mamba 选择性扫描使用解析估算。",
        "不计入 LayerNorm、GELU、Dropout、残差相加、reshape/indexing 和数据加载/预处理时间。",
        "",
        f"设备：`{device}`",
        f"Batch size：`{args.batch_sizes}`",
        f"重复次数/预热次数/迭代次数：`{args.repeats}/{args.warmup}/{args.iterations}`",
    ]
